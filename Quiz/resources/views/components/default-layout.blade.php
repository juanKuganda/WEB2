...
<body class="bg-zinc-100">
  <main>
    <header x-data="{ open: false }"
      ...
      <h2 class="text-lg sm:text-xl font-bold">Dashboard</h2>
      {{-- Desktop navigation --}}
      <nav>
      ...
      <div x-show="open" class="bg-white border border-zinc-300 shadow-lg sm:hidden absolute top-12 right-3">
        <ul class="flex flex-col gap-2">
          ...
          <li x-on:click="open = !open">
            <a href=""
               class="block px-4 py-2 text-zinc-600 text-sm hover:bg-gray-100">Pembeli</a>
          </li>
          <li x-on:click="open = !open">
            <a href="{{ route('Pakaian.index') }}"
               class="block px-4 py-2 text-zinc-600 text-sm hover:bg-gray-100">Pakaian</a>
          </li>
          <li x-on:click="open = !open">
            <a href=""
               class="block px-4 py-2 text-zinc-600 text-sm hover:bg-gray-100">Transaksi</a>
          </li>
        </ul>
      </div>
    </header>
